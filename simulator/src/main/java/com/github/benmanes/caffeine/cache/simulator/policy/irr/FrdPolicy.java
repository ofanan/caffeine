...
import com.github.benmanes.caffeine.cache.simulator.cache_mem_system.MyFrdPolicy; //$$
 ...
 */
public class FrdPolicy implements KeyOnlyPolicy {//$$ Removed "final", for letting other classes extend it
  ...
  public FrdPolicy(Config config) {
    ...
    this.data = new Long2ObjectOpenHashMap<>(); //The cache
  ...
  public static Set<Policy> policies(Config config) {
    return ImmutableSet.of(new MyFrdPolicy(config));
  }

//$$
  public boolean isInCache (Node node) {
	  return (node == null || node.status == Status.NON_RESIDENT)? false : true;
  }
  ...
  public void record(long key) {
    ...
    if (node == null) { // miss
      node = new Node(key);
      this.indicateInsertion(key); //$$ Debug comment: insert only here --> seems OK
      data.put(key,node);
      onMiss(node);
    } else if (node.status == Status.FILTER) { //hit
      onFilterHit(node);
    } else if (node.status == Status.MAIN) { //hit
      onMainHit(node);
    } else if (node.status == Status.NON_RESIDENT) { //miss $$ Debug comment: insert only here --> seems OK
      this.indicateInsertion(node.key); //$$
  ...
  private void onFullMiss(Node node) {
    ...
    this.indicateVictim(victim.key); //$$
