 ...
 */
public class SampledPolicy implements KeyOnlyPolicy { //$$ Removed "final", for letting other classes extend it
  ...
  public static Set<Policy> policies(Config config, EvictionPolicy policy) {
    ...
    return settings.admission().stream().map(admission ->
      new MySampledPolicy(admission, policy, config)
    ).collect(toSet());
  }
  ...
  private void evict(Node candidate) {
    if (data.size() > maximumSize) {
      ...
      if (admittor.admit(candidate.key, victim.key)) {
        ...
        this.indicateVictim(candidate.key); //$$
