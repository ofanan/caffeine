 ...
 */
public class WindowTinyLfuPolicy implements KeyOnlyPolicy { //$$ Removed "final", for letting other classes extend it
  private SetOfCBFs<Long> indicators; //$$ Added
  private final Long2ObjectMap<Node> data; // The cache
  ...
  public WindowTinyLfuPolicy(double percentMain, WindowTinyLfuSettings settings) {
  	String name = "Lfu"; // String name = String.format("sketch.WindowTinyLfu (%.0f%%)", 100 * (1.0d - percentMain)); // $$ Changed name to shorter and clearer one. 
  ...
  public void record(long key) {
    ...
    if (node == null) { // Node isn't cached, and no record is found for its frequency list 
      onMiss(key);
      policyStats.recordMiss();
    } else if (node.status == Status.WINDOW) {
      onWindowHit(node); // $$ Hit in the window - nothing to add / remove from the indicator
  ...
  private void onMiss(long key) {
    ...
    data.put(key, node);
    indicateInsertion (key);
    sizeWindow++;
    evict(); //$$ We will intercept the insertion / removal, if exists, within evict() 
  }
  ...
  private void onProbationHit(Node node) {
    ...
    //indicateInsertion (node.key); //$$ Indicate that the node is inserted to the protected region

    sizeProtected++;
    if (sizeProtected > maxProtected) {
      Node demote = headProtected.next;
//      indicateVictim (demote.key); //$$ "demote" will be evicted?
      demote.remove();
