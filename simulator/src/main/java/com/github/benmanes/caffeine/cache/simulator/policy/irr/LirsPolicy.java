...
import com.github.benmanes.caffeine.cache.simulator.cache_mem_system.MyLirsPolicy;
...
public class LirsPolicy implements KeyOnlyPolicy {//$$ Removed "final", for letting other classes extend it
  protected SetOfCBFs<Long> indicators; //$$ Added
  ...
  public LirsPolicy(Config config) {
    ...
    this.policyStats = new PolicyStats("Lirs"); //$$ Changed form "irr.lirs" to "Lirs"
  ...
  public static Set<Policy> policies(Config config) {
    return ImmutableSet.of(new MyLirsPolicy(config));
  }

  //$$
  public boolean isInCache (Node node) {
    return (node == null || (node.status == Status.HIR_NON_RESIDENT))? false : true;   
  }
  ...
  public void record(long key) {
    // policyStats.recordOperation(); $$ Commented, to short run-time
    Node node = data.get(key);
    indicators.handleRequest (key, isInCache(node)); //$$
    if (node == null) { // Node isn't cached, and no record is found for its frequency meta-data --> miss
      node = new Node(key);
      data.put(key,node); 
      onNonResidentHir(node); //indicateInsertion is called from within onNonResidentHir()
    } else if (node.status == Status.LIR) { //hit
      onLir(node);
    } else if (node.status == Status.HIR_RESIDENT) { //hit 
      onResidentHir(node); 
    } else if (node.status == Status.HIR_NON_RESIDENT) { //miss
  ...
  private void onLir(Node node) { //hit
  ...
  private void onResidentHir(Node node) { //hit
  ...
  private void onNonResidentHir(Node node) {
    ...
    this.indicateInsertion(node.key); //$$ The insertion policy is "always": every missed node is inserted to the $
  ...
  private void pruneStack() {
    ...
    while (sizeNR >  maximumNonResidentSize) {
      ...
      data.remove(removed.key); // $$ This is not a real victimization, but merely a removal from the meta-data of a key that is possibly not cached anyway.
    }
  }
  ...
  private void evict() {
    ...
    this.indicateVictim (bottom.key); //$$
