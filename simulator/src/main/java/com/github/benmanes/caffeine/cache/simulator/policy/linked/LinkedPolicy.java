...
public class LinkedPolicy implements Policy { //$$ Removed "final", for letting other classes extend it 
  ...
  public void record(AccessEvent event) {
    ...
    if (old == null) { // Miss
      ...
      if (weight > maximumSize) { // This single item is larger than the whole cache --> cannot cache it 
      ...
      node.appendToTail(); // Insert the item
      evict(node); // Evict item(s) (either the newly cached item, or an old one), if needed.
    } else { // Hit
  ...
  private void evict(Node candidate) {
    if (currentSize > maximumSize) { // Cache is full
      while (currentSize > maximumSize) {
        ...
        if (admit) { // Decided to admit the candidate, and victimize the victim
