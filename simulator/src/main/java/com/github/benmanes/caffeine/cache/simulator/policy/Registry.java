...
import com.github.benmanes.caffeine.cache.simulator.cache_mem_system.MyFrequentlyUsedPolicy; //$$
import com.github.benmanes.caffeine.cache.simulator.cache_mem_system.MyLinkedPolicy; //$$
import com.github.benmanes.caffeine.cache.simulator.cache_mem_system.MySampledPolicy;
...
public final class Registry {
  ...
  private static void registerMyLinked(Map<String, Function<Config, Set<Policy>>> factories) {
    ...
    // Uncomment the 4 lines below for simulating LinkedPolicy 
  ...
  private static void registerSampled(Map<String, Function<Config, Set<Policy>>> factories) {
	    Stream.of(MySampledPolicy.EvictionPolicy.values()).forEach(priority -> { //$$
	      String id = "sampled." + priority.name();
	      factories.put(id, config -> MySampledPolicy.policies(config, priority)); //$$
	    });
	  }
  ...
  private static void registerIrr(Map<String, Function<Config, Set<Policy>>> factories) {
    factories.put("irr.Frd", MyFrdPolicy::policies); //$$
